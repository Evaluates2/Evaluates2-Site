...
const languageDescriptionsMap = {
  JavaScript:
  "JavaScript, though mercilessly mocked by programmers across the board for years, somehow rose above and evolved to become king of the web. Its interpreted, dynamically-typed nature makes for very concise code, and it supports both class-based OOP and extremely functional coding styles. Virtually EVERY website and web application on the web today is written in or compiles down to JavaScript. Node.js is often our go-to choice both for simple lambda functions and complex backend servers. The ability to have one consistent language across front-end and back-end makes JavaScript an incredibly popular choice.",
  TypeScript:
  "Ohhh TypeScript... take all the things you like about JavaScript and add the ability to explicitly define interfaces, create enums, overload functions, etc. TypeScript gives developers syntactic sugar that \"should\" make your code easier to write, read, and understand. TypeScript sometimes gets a bad rap for causing annoying little errors and even sometimes slowing development in the short run, but after working with Evaluates2 engineers on a project with it we believe everyone can learn to love TypeScript!",
  'Clojure / ClojureScript':
  "Clojure is a dialect of lisp, and it aims to frame programming as a logical evaluation of functions and immutable data. It was created by Rich Hickey as a way to run Lisp on the JVM (first released in 2007). The syntax is meant to be extremely terse and clear as to allow programmers to get things done correctly, quickly, and enjoyably. A version that targets the Node.js V8 runtime, referred to as ClojureScript, appeared later as a response to the growing reach of JavaScript. Today, ClojureScript can be used on the back-end or front-end, allowing savvy Clojurists to build virtually anything they could make in JavaScript or Java with Clojure syntax instead. Here at Evaluates2 we appreciate the beauty and brevity of Clojure code, and we love building projects using this awesome language!",
  'Go Lang':
    "Go is an incredibly run-time efficient, statically-typed language that is great for server-side development. It is compiled directly to machine code and was created within Google in 2007 to improve developer productivity by replacing C++ code with something syntactically cleaner. When coding in Go you get a ton of power without all the hoopla, allowing programmers to quickly write code that is simple, efficient, and correct.",
  Rust:
    "Rust is an extremely fast, static typed language that emphasizes safe memory management. Created in 2006 within Mozilla to more easily develop safe systems, Rust has emerged as a very interesting, syntactically powerful language that compiles to machine code which can run at, near, and even is some cases better than the speed of equivalent code written in C or C++. Rust's actix framework often wins speed benchmarks for server-side code, and it's quickly becoming the language of choice for web assembly (WASM) front-end projects.",
  Swift:
  ...
  Java:
    "Java, released in 1995, was very popular langague at the turn of the century and still is today. Its object-oriented principles and straightforward yet sometimes overly-verbose syntax has been enjoyed by many developers over the years. Although its popularity has declined, new versions of Java are still being released in an attempt to compete with newer langauges that claim to have a cleaner and more succinct syntax. The JVM runtime is extremely fast for certain tasks and is still a great choice for many software projects today.",
  'C#':
  ...
  'Ruby':
    "Ruby is a language that was developed for programmer productivity with the idea that coding should be fun for programmers. It was developed in Japan in the mid 1990s and is still a popular choice for building quick prototypes and as a general purpose programming language.",
  'C / C++':
